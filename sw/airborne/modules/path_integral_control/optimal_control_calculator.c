 ...
 * Calculates the x and y optimal velocities. The probe construction and
 * the sampling procedure is included here.
 */
...
#define PI_NUM_PROBES 13  // 7 for the 180 deg case
#endif
#ifndef PI_FOLLOWING_ID
#define PI_FOLLOWING_ID 140   //the same drone is used as leader
#endif
...
bool pi_calc_timestep(struct path_integral_t *pi, struct pi_state_t *st, struct pi_wp_t *wp, struct pi_result_t *result){
  ...
  // Shift controls with dt for importance sampling
  ...
  // Choose the sampling method to be used: 0 - no probes 2 - with probes
  switch (pi->SAMPLING_METHOD){
...

    {
      ...
      // The initial velocity is now changed to the best probe one
  ...
  for (int n=0; n < pi->N; n++){
    ...
    for (int h=0; h < pi->iH; h++){
      ...
      // Apply discrete first order delay to the velocity
      float rise_time = 4;
...
/*            if(a != pi->following_id){
        ...
        case 3:
        {
          ...
          // Follow mouse's position !!! Mouse has the 0 index !!!
